...
WebUI「配置」页面提供「导出 JSON」和「导入 JSON」：

- 导出包含该页面全部可编辑配置项，包括 API Key、代理凭据和接码平台密钥。
- 导入只覆盖 JSON `config` 对象中出现的已知配置项，未知字段会被忽略。
- 导入成功后自动写入 `.env` 并热加载；WebUI 授权码、Session 签名密钥等启动级配置需要重启 WebUI 后完全生效。
- 导出文件不包含邮箱池、账号、Codex 凭证、任务或日志。

JSON 文件带有固定格式标识和版本号，适合在另一份项目实例的配置页直接导入。由于文件包含明文密钥，请按敏感配置文件保管，不要提交到 Git。
